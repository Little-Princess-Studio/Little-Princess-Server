...
namespace LPS.Common.Core.Rpc.RpcProperty.RpcContainer
{
        ...
        where TK : notnull
    {
        ...
        public static RpcPropertyContainer FromRpcArg(Any content)
        {
            ...
            if (content.Is(DictWithMailBoxKeyArg.Descriptor) && typeof(LPS.Common.Core.Rpc.MailBox) == typeof(TK))
            {
                var payload = content.Unpack<DictWithMailBoxKeyArg>().PayLoad;

                RpcDictionary<LPS.Common.Core.Rpc.MailBox, TV> rpcDict = new();
                Dictionary<LPS.Common.Core.Rpc.MailBox, RpcPropertyContainer> rawDict = new();
