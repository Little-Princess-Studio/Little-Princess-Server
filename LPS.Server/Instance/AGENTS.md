...
- Server-side shadow entities (v1 MVP, see `.sisyphus/plans/server-shadow-entity-v1.md`): a `DistributeEntity` subclass can be instantiated in shadow mode via `RpcServerHelper.CreateShadowEntityLocally`. The shadow lives in `Server.localShadowEntities` (keyed by derived `MailBox.Id` of form `oriId@shadow@ip:port:hostnum`), receives `PropertySyncCommandList` / `PropertyFullSync` via dedicated handlers in `Server.ShadowEntity.cs`, and REJECTS inbound `EntityRpc` (per design D1). The Gate maintains the subscription registry (`Gate.shadowSubscriptions`) and fans out property syncs to subscribed shadow-servers in `Gate.ClientMessages.cs`. v1 limitations: same-Gate only, no cross-Gate, no shadow RPC, no migration interaction, no crash recovery; see plan §"Non-goals".
